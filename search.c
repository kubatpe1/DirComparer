...
crawl_directories(struct search_context *context)
{
	...
	while ((current = stack_pop(&st)) != NULL) {
		...
		build_paths(current, context->source,
			    context->target, &first, &second);
		...
		while ((de = readdir(d)) != NULL) {
			...
			len = strlen(current);
			path = malloc((len + strlen(de->d_name) + 2)
				    * sizeof (char));
			strcpy(path, current);
			strcpy(path + len, "/");
			...
			free(first);
			free(second);
			build_paths(path, context->source,
				    context->target, &first, &second);
...
void
build_paths(char *rel_path, char *first, char *second,
	    char **first_res, char **second_res)
{
